...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        remain = self.volume - (len(self.contents) + len(additional_contents))
        extra = ''
        if remain < 0:

            for k in range(self.volume - len(self.contents)):
                self.contents = self.contents + additional_contents[k]
            for i in range(k+1,len(additional_contents)):
    ...
    def double(self):
        ...
        double_string = self.contents + self.contents
        remain = self.volume - (len(double_string))
        extra = ''
        if remain < 0:
            for k in range(self.volume - len(self.contents)):
                self.contents = self.contents + double_string[k]
            for i in range(k+1,len(double_string)//2):
                extra = extra + double_string[i]
        else:
            self.contents = double_string
            extra = ''
    ...
    def shrink(self, new_volume):
        ...
        discarded = self.contents[new_volume:self.volume]
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # ---------------------------------------------------------------------
        self.reset_contents = self.reset_contents + [self.contents]
    ...
    def steal(self, other_box):
        ...
        if self.volume - len(self.contents) > 0:
            s = other_box.contents[0:(self.volume - len(self.contents))]
            self.contents = self.contents + other_box.contents[0:(self.volume - len(self.contents))]

        else:
            s = ''
            self.contents = self.contents

        other_box.contents = other_box.contents[len(s):len(other_box.contents)]
